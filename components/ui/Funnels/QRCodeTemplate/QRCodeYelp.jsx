...
export default function QRCodeYelp() {
  const router = useRouter();
  const [hoveredRating, setHoveredRating] = useState(0);

  const handleRating = (value) => {
    localStorage.setItem("rating_qr_code", value.toString());
    if (value >= 4) {
      router.push("https://google.com/review");
    } else {
      router.push("/templates/standard/yelp/qr-code/reasons-for-low-rating");
    }
  };
